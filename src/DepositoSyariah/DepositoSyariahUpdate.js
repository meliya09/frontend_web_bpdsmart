...
export default function DepositoSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/depositosyariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/depositosyariah/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.depositosyariah.konten_menu)
          setKategori(result.depositosyariah.kategori_id)
          setKategoriNama(result.depositosyariah.kategori_nama)
