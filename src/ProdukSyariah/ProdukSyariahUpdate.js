...
export default function ProdukSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/syariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/syariah/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.syariah.konten_menu)
        setKategori(result.syariah.kategori_id)
        setKategoriNama(result.syariah.kategori_nama)
