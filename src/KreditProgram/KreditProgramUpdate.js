...
export default function KreditProgramUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/kreditprogram/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/kreditprogram/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.program.konten_menu)
          setKategori(result.program.kategori_id)
          setKategoriNama(result.program.kategori_nama)
