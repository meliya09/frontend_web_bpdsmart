...
export default function DanaKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/danakonven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/danakonven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.danakonven.konten_menu)
        setKategori(result.danakonven.kategori_id)
        setKategoriNama(result.danakonven.kategori_nama)
