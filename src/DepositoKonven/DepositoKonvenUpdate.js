...
export default function DepositoKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/depositokonven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/depositokonven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.depositokonven.konten_menu)
        setKategori(result.depositokonven.kategori_id)
        setKategoriNama(result.depositokonven.kategori_nama)
