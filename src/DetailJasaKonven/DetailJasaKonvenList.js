...
function DetailJasaKonvenList() {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://192.168.100.215:8080/detailjasakonven/${id}`)
        .then(res => {
          setDetail(res.data)
        })
    }
  ...
  const approve = async (id) => {
    await axios.post(`http://192.168.100.215:8080/detailjasakonven/approve/${id}`);
    setDetail();
  };
...
export default DetailJasaKonvenList;