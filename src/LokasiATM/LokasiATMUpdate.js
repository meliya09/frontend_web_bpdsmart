...
export default function LokasiATMUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/lokasiatm/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/lokasiatm/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setLokasiNama(result.lokasiatm.lokasi_nama)
        setKategori(result.lokasiatm.kategori_id)
        setKategoriNama(result.lokasiatm.kategori_nama)
