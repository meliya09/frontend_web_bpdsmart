...
function DetailTabKonvenList() {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://192.168.100.215:8080/detailtabkonven/${id}`)
        .then(res => {
          setDetail(res.data)
        })
    }
  ...
  const approve = async (id) => {
    await axios.post(`http://192.168.100.215:8080/detailtabkonven/approve/${id}`);
    setDetail();
  };
...
export default DetailTabKonvenList;