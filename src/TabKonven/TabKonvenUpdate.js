...
export default function TabKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/tabkonven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/tabkonven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.tabkonven.konten_menu)
        setKategori(result.tabkonven.kategori_id)
        setKategoriNama(result.tabkonven.kategori_nama)
