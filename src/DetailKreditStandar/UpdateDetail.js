...
export default function UpdateDetail({ open, handleClose }) {
    ...
    const [konten_promosi_gambar, setPromosiGambar] = useState("");
    const [konten_promosi_text, setPromosiText] = useState("");
    ...
    const getDetailById = async () => {
        const response = await axios.get(`http://192.168.100.215:8080/detailkreditstandar/${id}`);
        ...
        setPromosiText(response.data.konten_promosi_text);
        setPromosiGambar(response.data.konten_promosi_gambar);
    ...
    const loadGambar4 = (e) => {
        const konten_promosi_gambar = e.target.files[0];
        setPromosiGambar(konten_promosi_gambar);
    };
    ...
    const updateDetail = async (e) => {
        ...
        formData.append("konten_promosi_gambar", konten_promosi_gambar);
        formData.append("konten_promosi_text", konten_promosi_text);
        formData.append("konten_sk", konten_sk);
        try {
            await axios.post(`http://192.168.100.215:8080/detailkreditstandar/update/${id}`, formData, {
                headers: {
                ...
                .then(res => {
                    ...
                    window.location = '/kreditstandar/detailkreditstandar/' + id
                })
        } catch (error) {
            console.log(error);
        }
        try {
            await axios.post(`http://192.168.100.215:8080/detailkreditstandar/update2/${id}`, formData, {
                headers: {
                    "Content-type": "multipart/form-data",
                },
            })
        ...
        try {
            await axios.post(`http://192.168.100.215:8080/detailkreditstandar/update3/${id}`, formData, {
                headers: {
                    "Content-type": "multipart/form-data",
                },
            })
        ...
        try {
            await axios.post(`http://192.168.100.215:8080/detailkreditstandar/update4/${id}`, formData, {
                headers: {
                    "Content-type": "multipart/form-data",
                },
            })
                ...
                <DialogContent>
                    ...
                    >
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <InputLabel style={{ fontWeight: 'bold', color: '#1b5e20', fontSize: 16 }} >Gambar Promosi</InputLabel>
                                <input
                                    type="file"
                                    className="file-input"
                                    onChange={loadGambar4}
                                />
                            </Grid>
                            <Grid item xs={12}>
                            ...
                            <Grid item xs={12}>
                                <InputLabel style={{fontWeight: 'bold',color: '#1b5e20', fontSize: 16 }}>Promosi</InputLabel>
                                <JoditEditor
                                    ref={editor}
                                    value={konten_promosi_text}
                                    onChange={newContent => setPromosiText(newContent)}
                                />
                            </Grid>
                            <Grid item xs={12}>
