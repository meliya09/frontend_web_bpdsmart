...
export default function StandarLayananUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/standarlayanan/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/standarlayanan/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.standarlayanan.konten_menu)
        setKategori(result.standarlayanan.kategori_id)
        setKategoriNama(result.standarlayanan.kategori_nama)
