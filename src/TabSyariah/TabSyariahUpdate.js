...
export default function TabSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/tabsyariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/tabsyariah/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.tabsyariah.konten_menu)
          setKategori(result.tabsyariah.kategori_id)
          setKategoriNama(result.tabsyariah.kategori_nama)
