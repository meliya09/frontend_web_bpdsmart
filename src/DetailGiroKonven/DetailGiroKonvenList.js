...
function DetailGiroKonvenList() {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://192.168.100.215:8080/detailgirokonven/${id}`)
        .then(res => {
          setDetail(res.data)
        })
    }
  ...
  const approve = async (id) => {
    await axios.post(`http://192.168.100.215:8080/detailgirokonven/approve/${id}`);
    setDetail();
  };
...
export default DetailGiroKonvenList;