...
export default function JasaSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/jassasyariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/jasasyariah/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.jasasyariah.konten_menu)
        setKategori(result.jasasyariah.kategori_id)
        setKategoriNama(result.jasasyariah.kategori_nama)
