...
export default function IdentitasPribadiUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/identitas/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/identitas/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.identitas.konten_menu)
        setKategori(result.identitas.kategori_id)
        setKategoriNama(result.identitas.kategori_nama)
