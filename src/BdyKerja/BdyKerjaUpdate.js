...
export default function BdyKerjaUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/bdykerja/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/bdykerja/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.bdykerja.konten_menu)
        setKategori(result.bdykerja.kategori_id)
        setKategoriNama(result.bdykerja.kategori_nama)
