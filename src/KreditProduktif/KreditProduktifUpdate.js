...
export default function KreditProduktifUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/kreditproduktif/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/kreditproduktif/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.produktif.konten_menu)
          setKategori(result.produktif.kategori_id)
          setKategoriNama(result.produktif.kategori_nama)
