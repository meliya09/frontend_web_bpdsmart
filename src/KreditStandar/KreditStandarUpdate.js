...
export default function KreditStandarUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/kreditstandar/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/kreditstandar/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.standar.konten_menu)
          setKategori(result.standar.kategori_id)
          setKategoriNama(result.standar.kategori_nama)
