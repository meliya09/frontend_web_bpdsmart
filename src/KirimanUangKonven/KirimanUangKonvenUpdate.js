...
export default function KirimanUangKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/kirimanuangkonven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/kirimanuangkonven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.kirimankonven.konten_menu)
        setKategori(result.kirimankonven.kategori_id)
        setKategoriNama(result.kirimankonven.kategori_nama)
