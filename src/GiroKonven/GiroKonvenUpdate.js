...
export default function GiroKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/girokonven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/girokonven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.girokonven.konten_menu)
        setKategori(result.girokonven.kategori_id)
        setKategoriNama(result.girokonven.kategori_nama)
