...
export default function ModalKerjaUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/modalkerja/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/modalkerja/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.modalkerja.konten_menu)
        setKategori(result.modalkerja.kategori_id)
        setKategoriNama(result.modalkerja.kategori_nama)
