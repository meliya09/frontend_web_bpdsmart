...
export default function JasaKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/jasakonven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/jasakonven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.jasakonven.konten_menu)
        setKategori(result.jasakonven.kategori_id)
        setKategoriNama(result.jasakonven.kategori_nama)
