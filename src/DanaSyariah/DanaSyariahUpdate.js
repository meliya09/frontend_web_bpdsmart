...
export default function DanaSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/danasyariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/danasyariah/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.danasyariah.konten_menu)
          setKategori(result.danasyariah.kategori_id)
          setKategoriNama(result.danasyariah.kategori_nama)
        }
      )
  }, [id])
