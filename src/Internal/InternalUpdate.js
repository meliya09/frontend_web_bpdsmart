...
export default function InternalUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/internal/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/internal/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.internal.konten_menu)
        setKategori(result.internal.kategori_id)
        setKategoriNama(result.internal.kategori_nama)
