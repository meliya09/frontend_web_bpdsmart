...
export default function KreditKonsumerUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/kreditkonsumer/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/kreditkonsumer/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.konsumer.konten_menu)
          setKategori(result.konsumer.kategori_id)
          setKategoriNama(result.konsumer.kategori_nama)
