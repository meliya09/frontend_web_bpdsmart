...
export default function KirimanUangSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/kirimanuangsyariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/kirimanuangsyariah/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.kirimansyariah.konten_menu)
        setKategori(result.kirimansyariah.kategori_id)
        setKategoriNama(result.kirimansyariah.kategori_nama)
