...
function DetailLokasiATMList() {
  ...
  const LokasiDelete = async (id) => {
    await axios.delete(`http://192.168.100.215:8080/detaillokasiatm/${id}`);
  };

  useEffect(() => {
    if (id) {
      axios.get(`http://192.168.100.215:8080/lokasilayananatm/${id}`)
        .then(res => {
          setDetail(res.data)
        })
    }
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://192.168.100.215:8080/detaillokasiatm/${id}`)
        .then(res => {
...
export default DetailLokasiATMList;