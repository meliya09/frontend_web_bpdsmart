...
function DetailKreditProgramList() {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://192.168.100.215:8080/detailkreditprogram/${id}`)
        .then(res => {
          setDetail(res.data)
        })
    }
  ...
  const approve = async (id) => {
    await axios.post(`http://192.168.100.215:8080/detailkreditprogram/approve/${id}`);
    setDetail();
  };
...
export default DetailKreditProgramList;