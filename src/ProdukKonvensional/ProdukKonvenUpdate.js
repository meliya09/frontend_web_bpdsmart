...
export default function ProdukKonvenUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/konven/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/konven/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.konven.konten_menu)
        setKategori(result.konven.kategori_id)
        setKategoriNama(result.konven.kategori_nama)
