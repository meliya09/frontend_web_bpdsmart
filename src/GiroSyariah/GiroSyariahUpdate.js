...
export default function GiroSyariahUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/girosyariah/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Berhasil",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/girosyariah/` + id)
      .then(res => res.json())
      .then(
        (result) => {
          setKontenMenu(result.girosyariah.konten_menu)
          setKategori(result.girosyariah.kategori_id)
          setKategoriNama(result.girosyariah.kategori_nama)
