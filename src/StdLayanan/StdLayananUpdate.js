...
export default function StdLayananUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/stdlayanan/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/stdlayanan/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.stdlayanan.konten_menu)
        setKategori(result.stdlayanan.kategori_id)
        setKategoriNama(result.stdlayanan.kategori_nama)
