...
export default function PembiayaanUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/pembiayaan/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/pembiayaan/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.pembiayaan.konten_menu)
        setKategori(result.pembiayaan.kategori_id)
        setKategoriNama(result.pembiayaan.kategori_nama)
