...
export default function InformasiUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/informasi/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/informasi/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setKontenMenu(result.info.konten_menu)
        setKategori(result.info.kategori_id)
        setKategoriNama(result.info.kategori_nama)
