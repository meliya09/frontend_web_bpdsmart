...
export default function JaringanLayananUpdate() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(`http://192.168.100.215:8080/jaringanpelayanan/update/${id}`, data)
      .then((res) => {
        if (res.status === 200) {
          swal({
            title: "Success",
  ...
  useEffect(() => {
    fetch(`http://192.168.100.215:8080/jaringanpelayanan/`+id)
    .then(res => res.json())
    .then(
      (result) => {
        setLokasiNama(result.pelayanan.lokasi_nama)
        setKategori(result.pelayanan.kategori_id)
        setKategoriNama(result.pelayanan.kategori_nama)
