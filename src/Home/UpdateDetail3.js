...
export default function UpdateDetail3({ open3, handleClose3 }) {
    const classes = useStyles();
    const [konten_judul, setJudul] = useState("");
    const [konten_subjudul, setSubJudul] = useState("");
    const [konten_gambar3, setGambar] = useState("");
    const [konten_deskripsi3, setDeskripsi] = useState("");
    const { id } = useParams();
    ...
    const getDetailById = async () => {
        ...
        setGambar(response.data.konten_gambar3);
        setDeskripsi(response.data.konten_deskripsi3);
    };

    const loadGambar = (e) => {
        const konten_gambar3 = e.target.files[0];
        setGambar(konten_gambar3);
    };
    ...
    const updateDetail = async (e) => {
        ...
        formData.append("konten_deskripsi3", konten_deskripsi3);
        try {
            await axios.post(`http://192.168.100.215:8080/home/update3/${id}`, formData, {
                headers: {
        ...
        <div className={classes.paper} >
            <Dialog open={open3}
                ...
                <DialogContent>
                    ...
                    >
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                ...
                                <TextareaAutosize
                                    minRows={3}
                                    autoComplete="konten_deskripsi3"
                                    variant="outlined"
                                    id="konten_deskripsi3"
                                    value={konten_deskripsi3}
                    ...
                    <DialogActions>
                        <Button onClick={handleClose3} color='error' variant="contained">
                            Cancel
                        </Button>
                        <Button color="primary" onClick={updateDetail} handleClose={handleClose3} variant="contained">
                            Submit
                        </Button>
                    </DialogActions>
                </DialogContent>
